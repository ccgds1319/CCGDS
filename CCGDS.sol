...
contract CCGDS is ERC20, Ownable {
    ...
    uint256 public constant LIMIT_AMOUNT = 200 * 10**18; 
    ...
    function changeVaultWallet(address _address) external onlyOwner {
        require(_address != address(0), "Address cannot be null address");
        _isExcludedFromFees[vaultWallet] = false;

        vaultWallet = _address;
        excludeFromFees(vaultWallet, true);
    } 

    function changeFeeWallet(address _address) external onlyOwner {
        require(_address != address(0), "Address cannot be null address");
        _isExcludedFromFees[feeWallet] = false;
    ...
    function distributeToken() external {
        require(totalBurnedToken > 5000000 * 10**18, "Less than 5 million");
    ...
    function swapTokenAndFees(
        ...
        if(duration < ONE_DAY){
            ...
            if(stabilizeToken == 0) {
                if(totalBurnedToken < 500 * 10**22) {
                    stabilizeToken = 1000 * 10**22;
                } else {
            ...
            } else if(totalBurnedToken < 500 * 10**22) {
                burnAmount = amount.mul(7).div(100);  
                amount = amount.sub(burnAmount);
            } else {
    ...
    function swapAndSendToWalletsFee(uint256 tokenAmount) private {
        swapTokensForUSDT(tokenAmount, feeWallet);
    }
